...
and
  ...
  | Proj(e,id) ->
    eval_expr e >>= fun x -> let n = recordVals @@ x in (match List.assoc_opt id n with
    |None -> failwith "Field Not Found"
    |Some (ismutable, rval) -> if(ismutable)
    then return rval >>= int_of_refVal >>= Store.deref g_store else return rval) 
  | SetField(e1,id,e2) ->
    eval_expr e1 >>= fun x -> let fs = recordVals @@ x in (match List.assoc_opt id fs with
    |None -> failwith "Field Not Found"
    |Some (ismutable, rval) -> if(ismutable) then return rval >>= int_of_refVal >>= fun i -> eval_expr e2 >>= Store.set_ref g_store i >>= fun none -> return UnitVal else error "Field not mutable")
  | IsNumber(e) ->
    eval_expr e >>= is_numVal >>= fun x -> return (BoolVal x)
  | Unit -> return UnitVal
  | Debug(_e) ->
